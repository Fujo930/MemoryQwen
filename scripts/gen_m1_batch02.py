#!/usr/bin/env python3
"""M1 Batch 02 — Generate 100 model hardware route docs"""
from pathlib import Path

B = Path("megatrain/m1_model_hardware_extreme")
S = B / "sources"
S.mkdir(parents=True, exist_ok=True)

SLUG = """3B 跑通（smoke test/低资源验证），7B 常驻（默认推荐模型），14B 深度（complex reasoning/deep mode），32B+ 实验（不推荐 v0.1 默认家用路线）。MemoryQwen 的核心能力来自外部记忆系统和工作流，不是硬堆最大模型。模型越大不一定体验越好——还要考虑显存、KV cache、上下文空间、GPU Guardian 让路、游戏/创作体验。"""

for i in range(1, 101):
    title = f"Model Hardware Route 文档 {i:03d}：{'模型分层' if i%4==1 else ('硬件推荐' if i%4==2 else ('场景选择' if i%4==3 else '常见误解纠正'))}"
    body = f"""# {title}
...
v0.1, megatrain, m1, model_hardware, doc{i:03d}
"""
    (S / f"model_hardware_extreme_{i:03d}.md").write_text(body, encoding="utf-8")
