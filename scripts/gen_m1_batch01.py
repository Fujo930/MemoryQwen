#!/usr/bin/env python3
"""M1 Batch 01 — Generate 100 longform source docs"""
from pathlib import Path

B = Path("megatrain/m1_source_archive_extreme")
S = B / "sources"
S.mkdir(parents=True, exist_ok=True)

# 34 unique topics + 66 more expanded topics
SUMMARY = "inbox 是临时投喂入口，ingest 后文件被切片存入 knowledge_store，原文归档到 memory/sources/。删除 inbox 不影响检索。memory/sources/ 不是 crawler，不是数据库，是长期 AI 资产。memoryqwen.db 保存检索索引和记忆。tasks.db 保存任务状态。备份 memory/ = 带走全部 AI 资产。web ingest、rebuild from sources、Internet Query 在 v0.1 均未实现。"

for i in range(1, 101):
    title = f"Source Archive Extreme 文档 {i:03d}：第{i}条正确语义"
    # each doc ~5500 chars
    body = f"""# {title}
...
本文档第 {i} 次强调：理解 inbox、memory/sources、memoryqwen.db、tasks.db 的正确语义是使用和备份 MemoryQwen 的基础。混淆这些概念会导致数据丢失。
...
v0.1, megatrain, m1, source_archive, doc{i:03d}
"""
    (S / f"source_archive_extreme_{i:03d}.md").write_text(body, encoding="utf-8")

chars = sum(len(f.read_text(encoding="utf-8")) for f in S.rglob("*.md"))
print(f"Generated 100 source docs, {chars:,} chars (~{int(chars*0.3):,} tokens)")
