# MemoryQwen v0.1.5 能力边界对照手册

来源：MemoryQwen 训练资料
资料类型：training_rules
更新时间：2026-06-28
主题：v0.1.5 已实现 vs 未实现 vs 未来计划 — 精确对照表
关键词：capability boundary、Internet Query、联网、web search
...
| chat | ✅ 已实现 | 本地模型聊天 |
| Internet Query (联网查询) | ✅ v0.1.5 已实现 | web search/fetch/ask 获取临时网页资料，不是 crawler |
| chat --web | ✅ v0.1.5 已实现 | 自动判断是否需要联网 |
...
7. Internet Query ≠ crawler — web search/fetch 是受控查询，不递归抓取
8. web search/fetch/ask ≠ 自动写入记忆 — 只有 web ingest 显式命令才写入 memory
